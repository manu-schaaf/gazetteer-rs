...
impl Match {
    fn none() -> Self {
        Self {
            ...
            match_string: Vec::with_capacity(0),
            match_uri: String::new(),
        }
    }

    fn full(
        match_string: Vec<u32>,
        match_uri: String,
    ) -> Self {
        Match {
...
impl DisplayMatch {
    pub fn from_match(mtch: Match, tokenizer: &Tokenizer) -> DisplayMatch {
        let match_string = tokenizer.decode(&mtch.match_string).join(" ");
        DisplayMatch {
            match_type: mtch.match_type,
            match_uri: mtch.match_uri,
            match_string,
        }
    }
    pub fn from_matches(mtches: HashSet<Match>, tokenizer: &Tokenizer) -> Vec<DisplayMatch> {
        mtches.into_iter().map(|m| DisplayMatch::from_match(m, tokenizer)).sorted().collect()
    }
}

impl PartialOrd<Self> for DisplayMatch {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        Some(self.cmp(other))
    }
}

impl Ord for DisplayMatch {
    fn cmp(&self, other: &Self) -> Ordering {
...
impl HashMapSearchTree {
    ...
    pub fn load(&mut self, root_path: &str, generate_ngrams: bool, generate_abbrv: bool, filter_list: Option<&Vec<String>>) {
        ...
        let entries = segmented.into_iter().zip(lines.into_iter())
            .map(|(segments, (search_term, uri, match_type))| (segments.0, uri.clone(), match_type.clone()))
            .collect::<Vec<(Vec<u32>, String, MatchType)>>();
    ...
    fn load_entries(&mut self, entries: Vec<(Vec<u32>, String, MatchType)>, pb: Option<&ProgressBar>) {
        for (segments, uri, match_type) in entries {
            self.insert(VecDeque::from(segments.clone()), segments, uri, match_type);
    ...
    fn generate_ngrams(&self, lines: &Vec<(Vec<u32>, String, MatchType)>) -> Vec<(Vec<u32>, String, MatchType)> {
            ...
            .map(|(segments, uri, _)| {
                ...
                for ngram in ngrams {
                    ...
                    if tokenizer.decode(&ngram).iter().all(|el| el.len() > 2) {
                        result.push((ngram, String::from(uri), MatchType::NGram));
                    }
                }
                pb.inc(1);
                result
    ...
    // fn generate_abbreviations(&self, lines: &Vec<(Vec<u32>, String, MatchType)>) -> Vec<(Vec<String>, String, MatchType)> {
    //     let filtered = lines.par_iter()
    ...
    //             let mut result = Vec::new();
    //
    //             let head = String::from(tokenizer.decode(&vec![segments[0]]).get(0).unwrap());
    ...
    pub fn search<'a>(&self, text: &'a str, max_len: Option<usize>, result_selection: Option<&ResultSelection>) -> Vec<(String, Vec<DisplayMatch>, usize, usize)> {
        ...
        let mut results = slices
            ...
            .flatten()
            .map(|(s, mtches, a, b)| (s, DisplayMatch::from_matches(mtches, tokenizer), a, b))
            .collect::<Vec<(String, Vec<DisplayMatch>, usize, usize)>>();
...
impl HashMapSearchTree {
    fn from(match_string: Vec<u32>, match_uri: String) -> Self {
        Self {
    ...
    pub fn insert(&mut self, mut values: VecDeque<u32>, match_string: Vec<u32>, match_uri: String, match_type: MatchType) {
        let value: u32 = values.pop_front().unwrap();
