...
use crate::tree::{HashMapSearchTree, Match, MatchType, ResultSelection};
...
fn process_test_file(tree: &HashMapSearchTree, max_len: Option<i32>) {
...
fn process_test_output(results: Vec<(String, Vec<Match>, usize, usize)>) {
...
fn test_small_string_tree() {
    let mut tree = HashMapSearchTree::default();
    tree.load("resources/taxa.txt", false, 3, 2, None, false, &None);
...
fn test_big_string_tree() {
    let mut tree = HashMapSearchTree::default();
    tree.load("resources/BIOfid/*", false, 3, 2, None, false, &None);
    process_test_file(&tree, Option::from(5));
}
